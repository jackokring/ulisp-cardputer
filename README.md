...
You must add the `M5Cardputer`, `M5GFX`, `M5Unified` and `TinyGPSPlus` libraries to the Arduino project
to satisfy **all** build dependencies. As usual, the `Sketch > Export compiled Binary` option places the binary in the
project directory for use with `M5Launcher`.
...
I'm definitely not including everything, and some of the choices are based on making a useful tool foundation.
...
The esp32 board plugin must invalidate it in all likelihood, preventing function redefinition errors. I don't think the **keyword highlight** helps
...
- [ ] [Sound FX](/SFX.md) - the first seven channels are used (0 to 6), with channel 7 generating sound effects (if enabled), or just notes.
- [X] CardputerADV - I2C and `Serial1` safety code. Also `Serial2` via ADV top GPIO header.
- [X] GPS - serial option too. Pun.
...
* [MiniAcid](https://github.com/urtubia/miniacid) - ooh yes, tweaking for the ear.
* [M5Launcher](https://bmorcelli.github.io/Launcher/) - a nice fine app as occasionally used!
* [M5KeyboardMouse](https://github.com/geo-tp/Bluetooth-Keyboard-Mouse-Emulator) - press that GO for keys or mouse. USB and Bluetooth.
* [Ultimate-Remote](https://github.com/geo-tp/Ultimate-Remote) - apparently it does the lot.
* [WifiScan](https://github.com/geo-tp/Open-Wifi-Scanner) - clean show me the SSIDs.
